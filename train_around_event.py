...
parser.add_option('-e', '--gpsstop', dest="gpsstop", default=False, type='int', help='a GPS stop time for the event being analyzed. If default, gpsstop is calculated from the current time.')
...
parser.add_option('', '--gracedb-ID', type='string', help="ID for the GraceDB event being analyzed.")
...
for window in range(1,8):
    for (configname, thresh) in [("idqThresh25.ini", 25),("idqThresh30.ini", 30),("idqThresh35.ini", 35)]:
        gwthreshold = thresh
        config.read(configname)
        halfwind = window/2. * 60*60*24
        gpsstop = int(opts.gpsstop + halfwind)
        gpsstart = int(opts.gpsstop - halfwind)
        
        ### directory into which we write data
        output_dir = "%s/GraceDB%s/Window%u/DaysThresh%u/"%(traindir, opts.gracedb_ID, window, thresh)
        ...
        print(opts.gpsstop)
        proc = sp.Popen(["ligolw_segment_query_dqsegdb", "-t", "https://segments.ligo.org", "-q", "-a", "L1:DMT-ANALYSIS_READY:1", "-s", str(gps_start_time), "-e", str(gps_end_time), "-o", "sample_segments.xml", "-S"], stdout = sp.PIPE)
        ...
        trigger_dict.include([[gpsstart, gpsstop]])
        trigger_dict.include([[gpsstart, gpsstop]], channels=[gwchannel])
        ...
        if not opts.ignore_science_segments:
            ...
            clean_gps = sorted(event.randomrate(clean_rate, [[gpsstart, gpsstop]])) ### generate random clean times as a poisson time series within analysis range
