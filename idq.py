...
def build_auxmvc_vectors( trigger_dict, main_channel, time_window, signif_threshold, output_file_name, gps_start_time, gps_end_time, channels=None, unsafe_channels=None, science_segments=None, clean_times=None, clean_samples_rate=None, clean_window=None, filter_out_unclean=False, max_clean_samples=None, max_glitch_samples=None, verbose=False, locked_segments=None, include_time_locked=False, include_time_until_end=False):
    ...
    print("Number of trigs left after windowing: " + str(len(trigger_dict[main_channel])))
    # keep only triggers from the science segments if given........
    if science_segments:
        print 'Filtering through science segments.'
        science_segments = event.andsegments([[gps_start_time - time_window, gps_end_time + time_window]], science_segments)
        print(science_segments)
        trigger_dict.include(science_segments)
    print("Number of triggers left after filtering through science segments: " + str(len(trigger_dict[main_channel])))
    ...
    print 'Threshold to be applied:', signif_threshold
    ...
    aux_glitch_vecs = event.build_auxmvc_vectors(trigger_dict, main_channel=main_channel, coincidence_time_window=time_window, locked_segments=locked_segments)
    if (locked_segments):
        aux_glitch_vecs = enrich_auxmvc_vectors(aux_glitch_vecs, locked_segments, include_time_locked, include_time_until_end)
    ...
    aux_clean_vecs = event.build_auxmvc_vectors( trigger_dict, main_channel=main_channel, coincidence_time_window=time_window, build_clean_samples=True, clean_times=clean_times, clean_window=clean_window, locked_segments=locked_segments )
    if (locked_segments):
        aux_clean_vecs = enrich_auxmvc_vectors(aux_clean_vecs, locked_segments, include_time_locked, include_time_until_end)
...
def enrich_auxmvc_vectors(auxmvc_vectors, locked_segments, include_time_locked=False, include_time_until_end=False, include_time_normalized=False):
    """ Takes the numpy array generated in build_auxmvc_vectors and copies its information into an array with additional time_locked fields"""
    if not(include_time_locked or include_time_until_end):
        return auxmvc_vectors
    segs = tl.parse_segments(locked_segments) # extracts list of tuples (starttime, endtime) from locked-segments doc

    vector_size = auxmvc_vectors.size
    vector_names = auxmvc_vectors.dtype.names
    new_vector_names = []
    for name in vector_names:
        new_vector_names.append(name)
    if (include_time_locked):
        new_vector_names.append('time_locked')
    if (include_time_until_end):
        new_vector_names.append('time_end')
    new_vector_formats = ['g8', 'g8', 'g8', 'i'] + (len(new_vector_names)-4)*['g8']
    new_auxmvc_vectors = numpy.empty((vector_size, ), dtype = {'names':new_vector_names, 'formats':new_vector_formats})
    for i in range(auxmvc_vectors.size):
        for name in new_vector_names:
            if (name == 'time_locked'):
                time = tl.compute_time_since_locked(auxmvc_vectors['GPS'][i], segs) # computes time locked and places it into auxmvc_vectors
                new_auxmvc_vectors['time_locked'][i] = time
            elif (name == 'time_end'):
                time = tl.compute_time_until_end(auxmvc_vectors['GPS'][i], segs) # computes time until the end of segment and places it in auxmvc_vectors
                new_auxmvc_vectors['time_end'][i] = time
            else:
                new_auxmvc_vectors[name][i] = auxmvc_vectors[name][i]

    return new_auxmvc_vectors
